...
class NeuralNet:
    ...
    def calculate_square_of_past_grads(self):
        self.past_squared_grad_w1+=self.grad_w1**2
        self.past_squared_grad_bias1+=self.bias1**2
        self.past_squared_grad_w2+=self.grad_w2**2
        self.past_squared_grad_bias2+=self.bias2**2

    def back_propogation(self,target):
        self.calculate_gradients(target)
        self.calculate_square_of_past_grads()
        # update weights
        self.w1=self.w1 - ((self.lr/np.sqrt(self.past_squared_grad_w1+self.epsilon))*self.grad_w1)
        self.bias1=self.bias1 - ((self.lr/np.sqrt(self.past_squared_grad_bias1+self.epsilon))*self.grad_bias1)
        self.w2=self.w2 - ((self.lr/np.sqrt(self.past_squared_grad_w2+self.epsilon))*self.grad_w2)
        self.bias2=self.bias2 - ((self.lr/np.sqrt(self.past_squared_grad_bias2+self.epsilon))*self.grad_bias2)
