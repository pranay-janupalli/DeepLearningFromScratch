...
class NeuralNet:
    ...
    def initialize_parameters(self,num):
        self.w1=np.random.randn(num,num)
    ...
    def back_propogation(self,target):
        ...
        self.bias1=self.bias1 - ((self.lr*self.grad_bias1))
        self.w2=self.w2 - ((self.lr*self.grad_w2))
        self.bias2=self.bias2 - ((self.lr*self.grad_bias2))

    
    def train(self,data,targets,epochs):
        """
        Gradient descent algorithm updates the parameters 
        after every epoch.
        """
        for i in range(0,epochs):
            predictions=self(data)
            print(f"Loss of epoch: {i} is {self.loss(targets)}")
            self.back_propogation(targets)
