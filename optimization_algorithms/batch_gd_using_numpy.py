...
class NeuralNet:
    ...
    def back_propogation(self,target):
        ...
        # update weights
        self.w1=self.w1 - ((self.lr*self.grad_w1)+(self.lr*self.weight_decay*self.w1))
        self.bias1=self.bias1 - ((self.lr*self.grad_bias1)+(self.lr*self.weight_decay*self.bias1))
        self.w2=self.w2 - ((self.lr*self.grad_w2)+(self.lr*self.weight_decay*self.w2))
        self.bias2=self.bias2 - ((self.lr*self.grad_bias2)+(self.lr*self.weight_decay*self.bias2))
    ...
    def train(self,data,targets,epochs,valid_data,valid_targets):
        """
        Batch Gradient Descent updates the weight every batch forward pass
        """
...
model=NeuralNet(5)
model.set_learning_rate(1e-5,0.0001)
model.set_weight_decay(0.0001)
model.train(train_x,train_y,6,valid_x,valid_y)